# A Packer template to deploy a Red Hat Enterprise Linux 10 VM using the vsphere-supervisor builder.
# Supports ISO import (from a URL-accessible image) pattern for the RHEL 10 boot ISO.
# 
# ISO file (from project iso/ directory):
#   rhel-10.0-x86_64-dvd.iso
#
# Prerequisites:
#   - The RHEL 10 ISO must be accessible via HTTPS/HTTP URL from the
...
#   - A cloud-init bootstrap data file (default: ./cloud-init-rhel-10.yml) should
...
#     -var="image_name=rhel-10-0-template" \
#     -var="supervisor_namespace=my-namespace" \
#     rhel-10-0.pkr.hcl
#
# Usage (ISO hosted on a remote server):
#   packer build \
#     -var="import_source_url=https://your-storage.example.com/rhel-10.0-x86_64-dvd.iso" \
#     -var="class_name=best-effort-xsmall" \
#     -var="storage_class=m01-cl01-optimal-datastore-default-policy-raid5" \
#     -var="image_name=rhel-10-0-template" \
#     -var="supervisor_namespace=my-namespace" \
...
# ---------------------------------------------------------------------------
# Project ISO filename (defaults to the RHEL 10 ISO in iso/)
# ---------------------------------------------------------------------------
...
variable "import_source_url" {
  type        = string
  default     = null
  description = "URL to the RHEL 10 ISO image. Leave null when using use_local_iso_server=true."
}
...
variable "bootstrap_data_file" {
  type        = string
  default     = "./cloud-init-rhel-10.yml"
...
source "vsphere-supervisor" "rhel-10" {
...
build {
  sources = ["source.vsphere-supervisor.rhel-10"]

  # Initial connectivity check
  provisioner "shell" {
    inline = [
      "echo 'SSH connection established. Starting RHEL 10 provisioning.'",
      "hostnamectl",
  ...
  provisioner "shell" {
    only = ["vsphere-supervisor.rhel-10"]
    inline = [
  ...
  provisioner "shell" {
    inline = [
      "echo '========================================'",
      "echo 'RHEL 10 template build complete!'",
