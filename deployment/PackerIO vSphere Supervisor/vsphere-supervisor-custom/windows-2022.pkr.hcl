# A Packer template to deploy a Windows Server 2022 VM using the vsphere-supervisor builder.
...
#
# Prerequisites:
#   - The Windows Server 2022 ISO must be accessible via HTTPS/HTTP URL from the
...
#     -var="image_name=windows-2022-template" \
#     -var="supervisor_namespace=my-namespace" \
#     windows-2022.pkr.hcl
#
# Usage (ISO hosted on a remote server):
#   packer build \
#     -var="import_source_url=https://your-storage.example.com/SW_DVD9_Win_Server_STD_CORE_2022_2108.3_64Bit_English_DC_STD_MLF_X22-94021.ISO" \
#     -var="class_name=best-effort-xsmall" \
#     -var="storage_class=thick" \
#     -var="image_name=windows-2022-template" \
#     -var="supervisor_namespace=my-namespace" \
...
# ---------------------------------------------------------------------------
# Project ISO filename (defaults to the Windows Server 2022 ISO in iso/)
# ---------------------------------------------------------------------------
variable "iso_filename" {
  type        = string
  default     = "SW_DVD9_Win_Server_STD_CORE_2022_2108.3_64Bit_English_DC_STD_MLF_X22-94021.ISO"
...
variable "import_source_url" {
  type        = string
  default     = null
  description = "URL to the Windows Server 2022 ISO or OVA image. Leave null when using use_local_iso_server=true."
}
...
variable "bootstrap_data_file" {
  type        = string
  default     = "./sysprep-unattend-windows-2022.yml"
...
# Product key for Windows Server 2022
variable "windows_product_key" {
  type        = string
  default     = ""
  sensitive   = true
  description = "Windows Server 2022 product key (leave empty for evaluation/trial)"
}
...
source "vsphere-supervisor" "windows-2022" {
...
build {
  sources = ["source.vsphere-supervisor.windows-2022"]

  # Wait for WinRM to be ready
  provisioner "powershell" {
    inline = [
      "Write-Host 'WinRM connection established. Starting Windows Server 2022 provisioning.'",
  ...
  provisioner "powershell" {
    only = ["vsphere-supervisor.windows-2022"]
    inline = [
      "if (\"${var.windows_product_key}\" -ne \"\") {",
      "  Write-Host 'Applying Windows Server 2022 product key'",
      "  $productKey = \"${var.windows_product_key}\"",
      "  if (Test-Path 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\SoftwareProtectionPlatform') {",
      "    $result = cscript //nologo C:\\Windows\\System32\\slmgr.vbs /ipk $productKey",
      "    Write-Host \"Product key result: $result\"",
      "  }",
      "}"
    ]
  }
  ...
  provisioner "powershell" {
    inline = [
      "Write-Host '========================================'",
      "Write-Host 'Windows Server 2022 template build complete!'",
