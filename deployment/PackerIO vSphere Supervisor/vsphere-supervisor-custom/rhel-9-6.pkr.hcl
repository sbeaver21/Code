# A Packer template to deploy a Red Hat Enterprise Linux 9.6 VM using the vsphere-supervisor builder.
# Supports ISO import (from a URL-accessible image) pattern for the RHEL 9.6 boot ISO.
# 
# ISO file (from project iso/ directory):
#   rhel-9.6-x86_64-dvd.iso
#
# Prerequisites:
#   - The RHEL 9.6 ISO must be accessible via HTTPS/HTTP URL from the
...
#     -var="image_name=rhel-9-6-template" \
#     -var="supervisor_namespace=my-namespace" \
#     rhel-9-6.pkr.hcl
#
# Usage (ISO hosted on a remote server):
#   packer build \
#     -var="import_source_url=https://your-storage.example.com/rhel-9.6-x86_64-dvd.iso" \
#     -var="class_name=best-effort-xsmall" \
#     -var="storage_class=m01-cl01-optimal-datastore-default-policy-raid5" \
#     -var="image_name=rhel-9-6-template" \
#     -var="supervisor_namespace=my-namespace" \
...
# ---------------------------------------------------------------------------
# Project ISO filename (defaults to the RHEL 9.6 ISO in iso/)
# ---------------------------------------------------------------------------
...
variable "import_source_url" {
  type        = string
  default     = null
  description = "URL to the RHEL 9.6 ISO image. Leave null when using use_local_iso_server=true."
}
...
build {
  ...
  provisioner "shell" {
    inline = [
      "echo 'SSH connection established. Starting RHEL 9.6 provisioning.'",
      "hostnamectl",
  ...
  provisioner "shell" {
    inline = [
      "echo '========================================'",
      "echo 'RHEL 9.6 template build complete!'",
