...
packer {
  ...
  required_plugins {
    ...
    # Ansible plugin for Windows configuration management
    ansible = {
      source  = "github.com/hashicorp/ansible"
      version = ">= 1.1.1, < 2.0.0"  # Pin to major version to avoid breaking changes
    }
    # Windows Update plugin for applying Windows updates during build
    windows-update = {
      source  = "github.com/rgl/windows-update"
      version = "~> 0.16.10"  # Use pessimistic operator for patch-level updates
    }
  }
}
...
locals {
  ...
  iso_paths = {
    content_library = "es10-WLD1-ContentLibrary/Windows2022/windows2022.iso",
    datastore       = "[es00vx-w1c1-vsan] efa1d460-90f1-8300-1738-0c42a1cbef98/SW_DVD9_Win_Server_STD_CORE_2022_2108.3_64Bit_English_DC_STD_MLF_X22-94021.ISO"
  ...
  vm_name_datacenter_core    = "windows-server-2022-datacenter-core-${local.build_version}"
  vm_name_datacenter_desktop = "windows-server-2022-datacenter-dexp-${local.build_version}"
  vm_name_standard_core      = "windows-server-2022-standard-core-${local.build_version}"
  vm_name_standard_desktop   = "windows-server-2022-standard-dexp-${local.build_version}"
  bucket_name                = replace("windows-server-2022", ".", "")
  bucket_description         = "windows server 2022"
}
...
source "vsphere-iso" "windows-server-standard-2022-core" {
...
source "vsphere-iso" "windows-server-standard-2022-dexp" {
...
source "vsphere-iso" "windows-server-datacenter-2022-core" {
...
source "vsphere-iso" "windows-server-datacenter-2022-dexp" {
  ...
  cd_content = {
    "autounattend.xml" = templatefile("${abspath(path.root)}/data/autounattend.pkrtpl.hcl", {
      ...
      vm_inst_os_language  = "en-US"
      vm_inst_os_keyboard  = "en-US"
      vm_inst_os_image     = "Windows Server 2022 SERVERDATACENTER"
      vm_inst_os_key       = "XXXXX-XXXXX-XXXXX-XXXXX-XXXXX"
      vm_guest_os_language = "en-US"
      vm_guest_os_keyboard = "en-US"
      vm_guest_os_timezone = "UTC"
